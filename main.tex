...
\begin{lemma}\label{lem:finitetuple}
    For every finite semigroup $S$, there exists a function $\delta : \{0,1\}^{|S|} \rightarrow S$ such that for every $B \subseteq S$ and $\gamma : \{0,1\}^k \rightarrow S$, the quantifier $\Gamma^{S,B}_{\gamma}$ is definable in $(\Gamma^{S,B}_{\delta})[\varnothing]$.
  \end{lemma}
  \begin{proof}
    Recall that $\Gamma^{S,s}_{\gamma}$ is the class of structures $\str{A}$ in a vocabulary $\tau$ with one binary relation $<$ and $k$ unary relations $R_1,\ldots,R_k$ such that $\gamma(w_{\str{A}}) = s$ where $w_{\str{A}}$ is the string associated with $\str{A}$ as in Def.~\ref{def:associated}.
...
Hence, $\delta(w_{I(\str{A})}) = \gamma(w_{\str{A}})$ and therefore $I(\str{A}) \in \Gamma^{S,B}_{\delta}$ if, and only if, $\str{A} \in \Gamma^{S,B}_{\gamma}$ as required.
  ...
  It then follows from Lemma~\ref{lem:finitetuple} and the substitution property of quantifiers that the expressive power of $(\FO \cup \Gamma^{S})[\mathfrak{N}]$ is the same as that of $(\FO \cup \Gamma^{S}_{\delta})[\mathfrak{N}]$.  Indeed, any application of a quantifier in $\Gamma^S$ to an interpretation of dimension $d$ can be replaced by an application of a quantifier  $\Gamma^{S,B}_{\delta}$ to an interpretation of the same dimension.  We next aim to show that an application of a quantifier $\Gamma^{S,B}_{\delta}$ to an interpretation of dimension $d$ can be replaced by $d$ nested applications of quantifiers to interprettaions of dimension $1$.

  \begin{lemma}\label{lem:nesting}
    For any collection $\mathfrak{Q}$ of quantifiers, any formula of $(\mathfrak{Q}  \cup \Gamma^{S}_{\delta})[\mathfrak{N}]$ is equivalent to one of $(\mathfrak{Q}  \cup \Gamma^{S}_{1,\delta})[\mathfrak{N}]$.
  \end{lemma}
  \begin{proof}
Again, fix an enumeration $S = \{s_1,\ldots,s_c\}$ of $S$ and recall that $\delta : \{0,1\}^c \rightarrow S$ takes the one-hot encoding of $i$ to $s_i$.
    
    We show, by induction on $d$ that if we have a formula
    \[ \Phi := \Gamma^{S,B}_{\delta} x_1,\ldots,x_d (\phi_1(x_1,\ldots,x_d),\ldots,\phi_c(x_1,\ldots,x_d)) \]
 where each formula $\phi_i$ is in $(\mathfrak{Q}  \cup \Gamma^{S}_{1,\delta})[\mathfrak{N}]$, then $\Phi$  is equivalent to a formula of $(\mathfrak{Q}  \cup \Gamma^{S}_{1,\delta})[\mathfrak{N}]$.  The result then follows by induction on the structure of the formula.
    ...
    \[ \Phi_1 := \Gamma_{\delta}^{S,B} x_1  (\theta(x_1),\ldots,\theta(x_1)) \]
    where $\theta_i$ is the formula
    \[ \theta_i(x_i) := \Gamma_{\delta}^{S,s_i}x_2,\ldots,x_d (\phi_1(x_1,\ldots,x_d),\ldots,\phi_c(x_1,\ldots,x_d)).\]

    Thus, $\Phi_1$ is obtained as the application of $ \Gamma_{\delta}^{S,B}$ to a unary interpretation $I_1 := (\theta(x_1),\ldots,\theta(x_1))$ where each formula $\theta_i$ is obtained as the application of a quantifier $ \Gamma_{\delta}^{S,s}$ to an interpretation of dimension $d-1$ defined by formulas of $(\FO \cup \Gamma^{S}_{1,\delta})[\mathfrak{N}]$.  Thus, by induction hypothesis, each $\theta_i$ is equivalent to a formula of $(\mathfrak{Q}  \cup \Gamma^{S}_{1,\delta})[\mathfrak{N}]$ and we are done.

      It remains to show that $\Phi$ and $\Phi_1$ are equivalent in any string $\str{A}$.  To see this, fix an assignment $\alpha$ of values in $|\str{A}|$ to the free variables of $\Phi$.  We need to show that $\str{A} \models \Phi[\alpha]$ if, and only if, $\str{A} \models \Phi[\alpha]$.  Let $n$ be the length of $\str{A}$ and assume without loss of generality that the elements of $|\str{A}|$ are $\{1,\ldots,n\}$ in that order.
...
\]
    ...
    \begin{theorem}\label{thm:finitebinding}
         For every finite semigroup $S$, there exists a function $\delta : \{0,1\}^{|S|} \rightarrow S$ such that for any collection of quantifiers $\mathfrak{Q}$ and any set $\mathfrak{N}$ of numerical predicates, every formula of $(\mathfrak{Q} \cup \Gamma^S)[\mathfrak{N}]$ is equivalent to a formula of $(\mathfrak{Q} \cup \Gamma^S_{1,\delta})[\mathfrak{N}]$.
   \end{theorem}
   \begin{proof}
     Let $\phi$ be any formula of $(\mathfrak{Q}  \cup \Gamma^S)[\mathfrak{N}]$.  By Lemma~\ref{lem:finitetuple} we can replace all occurrences of quantifiers $\Gamma^{S,B}_{\gamma}$ by their definitions using $\Gamma^{S,B}_{\delta}$ to get  formula $\phi'$ of $(\mathfrak{Q}  \cup \Gamma^S_{\delta})[\mathfrak{N}]$ equivalent to $\phi$.  Finally, by Lemma~\ref{lem:nesting}, there is a formula of $(\mathfrak{Q}  \cup \Gamma^S_{1,\delta})[\mathfrak{N}]$ equivalent to $\phi'$.
\end{proof}
